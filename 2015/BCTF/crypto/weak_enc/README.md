...
The challenge consists of a python daemon running an encryption service using a custom encryption algorithm. Upon every connection to the daemon, we are sent a challenge and have to respond with a small 'proof-of-work' consisting of finding the pre-image (the first 12 bytes of which consist of the challenge) for a SHA1 hash where the lower-order 16 bits are all 0:
...
This gives us an adaptive (partially) chosen plaintext scenario (since every message is prefixed with an unknown, but static, salt and we can adapt our choice of plaintext based on previous ciphertext responses). The attack, however, is not as straightforward as simply bruteforcing the plaintext message and comparing it against the target ciphertext since the proof-of-work challenge-response mechanism makes this an infeasible approach.
